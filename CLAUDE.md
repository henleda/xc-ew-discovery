...
4. **`model/event.go` is frozen.** Adding, removing, renaming, or retyping a field requires an entry in `docs/DECISIONS.md` written first.
...
Six components. Each writes `model.Observation` and nothing else.
...
| `scrape/k8s/` | Gateway API HTTPRoute and GRPCRoute, Ingress. Declared inventory. |
...
- Do not add top-level packages. The seven above are the structure. Add subpackages instead.
